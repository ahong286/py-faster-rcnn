...
def vis_detections(
    ...
    save_to=None, image_name=None, model_name=None, net=None,
    ...
    if len(inds) == 0:
        ...
        plt.axis('off')
        if save_to:
            save_images(save_to, image_name)
        return
...
##
def demo(model, model_name, net, image_name, save_to=None):
    ...
    for cls_ind, cls_val in enumerate(CLASSES[1:]):
	    ...
	    im=im, class_name=cls_val, dets=dets, thresh=CONF_THRESH,
            save_to=save_to, image_name=image_name, model_name=model_name, net=model,
            fig=fig, ax=ax
        )
    plt.close("all")
...
if __name__ == '__main__':
    ...
    ##
    # Load data
    if imdb.endswith(".txt"):
            print("Reading images from txt file.")
            im_names = [im_name.rstrip('\n') for im_name in open(imdb)]

    elif os.path.isdir(imdb):
    ...
    for im_name in im_names:
        ...
        demo(prototxt, modelname, net, im_name, save_to)
